 ...
 *
 * To generate the goal, the goal generator needs to know the ego motion and
 * non-ego motions on the road, since it has to adjust the target speed and
 * target lane for the planner.
 */
class GoalGenerator {
 public:
  explicit GoalGenerator(const NaviMap &map) : _map(map) {}

  Goal GenerateGoal(const BehaviorState &behaviorState,
                    const double speedReference,
                    const SensorFusions &sensorFusions,
                    const CarState &carState, const Path &prevPath);

 private:
  const NaviMap &_map;

  Goal _GenerateLKGoal(const double speedReference,
                       const SensorFusions &sensorFusions,
                       const CarState &carState, const Path &prevPath);

  Goal _GenerateLCPGoal(int direction, const double speedReference,
                        const SensorFusions &sensorFusions,
                        const CarState &carState, const Path &prevPath);

  Goal _GenerateLCGoal(int direction, const double speedReference,
                       const SensorFusions &sensorFusions,
                       const CarState &carState, const Path &prevPath);
};
...
class PathPlanner {
 public:
  PathPlanner(const NaviMap &map) : _map(map) {}
  ...
  double EvaluatePath(const Goal &goal, const Path &path,
 ...
 private:
  ...
  float _ComputeSpeedScore(const Goal goal,
                           const std::array<double, 3> &averSpeeds) {
  ...
  Path _GeneratePath(const double targetDValue, const double targetSValue,
                     const double speedReference);
